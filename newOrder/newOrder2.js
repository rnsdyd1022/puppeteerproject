...
const updateBoxNum = async num => {
  ...
  var pageSelector = "body > fg-root > div.fg-container > fg-secure-layout > div > div.fg-content > fg-orders > div:nth-child(4) > div > fg-order-list > div.panel__body.panel__body--nopadding.is-active > div > div > div.table-grid__center.align-mid.width-40p > fg-pagination > ul > li.pagination__numbers > div > div.table-grid__center.align-mid > div";
  var ps = await self.page.$(pageSelector);
  var pages = await self.page.evaluate(ps => parseInt(ps.innerText.substring(3)), ps);
